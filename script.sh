...
#
# 描述: 此脚本不仅实现了核心参数的精简优化与硬件的动态适配，
#       还具备自动备份管理功能，每次运行会创建新备份并自动清理
#       更早的备份，确保配置目录的整洁。
# ==============================================================================
...
manage_backups() {
    ...
    if [ -f "$CONF_FILE" ]; then
        ...
        echo -e "${YELLOW}>>> 创建新备份到: $BAK_FILE${NC}"
...
EOF
    add_conf "net.core.default_qdisc" "fq" "使用Fair Queue调度器，配合BBR效果更佳"
...
show_tips() {
    ...
    if [ -n "$bak_file_hint" ]; then
        echo -e "${YELLOW}要撤销优化，请运行以下命令恢复最新备份:${NC}"
